...
export const TeacherProfile = () => {
    ...
    const getUserFields = (item) => {
        const fields = [
            ...
            { label: 'Number Document', value: item.numberDocument },
            { label: 'Phone', value: item.phone },
            { label: 'Username', value: item.username, roles: ['user', 'teacher'] },
            { label: 'Gender', value: item.gender, roles: ['user', 'teacher'] },
            { label: 'Certificate', value: item.certificateTeacher, roles: ['teacher'] },
        ];

        return fields.filter(field => !field.roles || field.roles.includes(store.currentRole));
    };
            ...
            <table className="table mx-auto">
                ...
                <tbody>
                    {store.user[`access_to_${store.currentRole}`].map((item, index) => (
                        item.email === userToLogin.email && (
                            <React.Fragment key={item.id || index}>
                                {getUserFields(item).map((field, idx) => (
                                    <tr key={idx}>
                                ...
                                <tr>
                                    <th scope="row">Actions</th>
                                    <td>
                                        <button className="btn btn-primary" onClick={() => handleUpdateUser(item.id)}>Edit</button>
                                    </td>
                                </tr>
                            </React.Fragment>
                        )
