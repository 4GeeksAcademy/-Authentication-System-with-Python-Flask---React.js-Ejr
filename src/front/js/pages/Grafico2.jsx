...
function Grafico2() {
	...
	const fetchData = async () => {
		...
		let result = await callAPI("https://api.coingecko.com/api/v3/coins/cardano/market_chart?vs_currency=clp&days=1&interval=1m");
	...
	const initChart = (data) => {
		let trace_price = {
			...
			type: "scatter",
			mode: "lines+markers",
			marker: { color: "rgb(140,82,255)", size: 3 },
		};
		let trace_volumes = {
			name: "Volumen ($B)",
			x: data.index.map((t) => new Date(t)),
